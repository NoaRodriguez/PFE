...
import { Plus, Calendar as CalendarIcon, Trophy, Pencil, Trash2 } from 'lucide-react';
...
export default function CalendarPage({ onNavigate }: CalendarPageProps) {
  ...
  const allEvents = [
    ...
    ...competitions.map((c) => ({ ...c, type: 'competition' as const })),
  ].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()); // Ordre chronologique (du plus proche au plus éloigné)
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950 pb-24">
      ...
      <div className="max-w-md mx-auto p-4 space-y-4">
        {/* Calendar View */}
        <VisualCalendar sessions={sessions} competitions={competitions} />
        ...
        {/* Events List */}
        <div>
          <h2 className="text-gray-900 dark:text-white mb-3 text-lg">Événements récents</h2>
          <div className="space-y-3">
            {allEvents.length === 0 ? (
              <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 text-center shadow-lg border border-gray-100 dark:border-gray-800">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Aucune séance ou compétition planifiée</p>
              </div>
            ) : (
              allEvents.slice(0, 5).map((event) => (
                ...
                >
                  {event.type === 'session' ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-600 dark:text-gray-400">
                            {new Date(event.date).toLocaleDateString('fr-FR')} • {event.duration} min
                          </p>
                        </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-[#F57BFF]/20 to-[#C1FB00]/20 rounded-xl flex items-center justify-center flex-shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-600 dark:text-gray-400">
                            {new Date(event.date).toLocaleDateString('fr-FR')} • {event.distance} km
                          </p>
                        </div>
